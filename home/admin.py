...
class DsaformAdmin(ImportExportActionModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        if Dsaform.objects.filter(teachername=obj.teachername).exists():
            messages.error(request, "The user already exists. Please modify the existing user for required changes. ")
        else:
    ...
    def download_dsaform(self, request, queryset):
        ...
        if queryset.__len__() > 1:
            ...
            teacher_name = queryset[0].teachername
            teacher_name = teacher_name.teacher_name
            dep_name = queryset[0].dep_name

            sub = Subject.objects.filter(dsaform__teachername__teacher_name=teacher_name)
            ...
            for i in range(0, sub_len):
                ...
                subject = str(s.subject_name.subject_name)
